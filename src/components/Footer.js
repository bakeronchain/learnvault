...
function Footer() {
    ...
    return (<footer className="px-6 py-12 mt-20 border-t border-white/5 relative overflow-hidden">
            ...
            },
            {
        ...
        ].map(function (link) { return (<a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" className="text-xs font-black uppercase tracking-widest text-white/30 hover:text-brand-cyan transition-colors">
							{link.label}
						</a>); })}
				</div>
				...
				<div className="flex items-center gap-6">
					...
					<div className="flex items-center gap-2 px-4 py-2 glass rounded-xl border border-white/5">
						...
						<span className="text-[10px] font-black uppercase tracking-[2px] text-white/40">
							Powered by Soroban
						</span>
					</div>
				</div>
			</div>
			<p className="text-center text-[10px] text-white/10 mt-12 uppercase tracking-[4px] font-black">
