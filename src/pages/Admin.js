...
var Admin = function () {
    var _a = (0, react_1.useState)("courses"), activeSection = _a[0], setActiveSection = _a[1];
    ...
    if (!isAdmin)
        return null;
    return (<div className="flex min-h-screen text-white relative overflow-hidden">
			{/* Background Accent */}
			...
			<aside className="w-72 glass border-r border-white/5 p-8 flex flex-col gap-10 z-10">
				...
				<nav className="flex flex-col gap-3">
					{["courses", "milestones", "users", "treasury", "contracts"].map(function (section) { return (<button key={section} className={"w-full text-left px-5 py-3.5 rounded-xl capitalize transition-all duration-300 relative group overflow-hidden ".concat(activeSection === section
                ? "text-brand-cyan font-bold"
                : "text-white/40 hover:text-white")} onClick={function () { return setActiveSection(section); }}>
								{activeSection === section && (<div className="absolute inset-0 bg-white/5 animate-pulse"/>)}
								<span className="relative z-10 flex items-center gap-3">
									<span className={"w-1.5 h-1.5 rounded-full transition-all duration-300 ".concat(activeSection === section ? "bg-brand-cyan scale-125 shadow-[0_0_10px_rgba(0,210,255,0.8)]" : "bg-transparent")}/>
									{section}
								</span>
							</button>); })}
				</nav>
...
var CourseManagement = function () {
    ...
    var addCourse = function () {
        ...
        setCourses(__spreadArray(__spreadArray([], courses, true), [newCourse], false));
    };
    return (<section className="glass-card p-10 rounded-[2.5rem]">
			<div className="flex justify-between items-center mb-12">
				...
				<button className="px-8 py-3 bg-linear-to-r from-brand-cyan to-brand-blue rounded-full font-black text-sm hover:scale-105 active:scale-95 transition-all shadow-xl shadow-brand-cyan/20 uppercase tracking-widest" onClick={addCourse}>
					+ New Course
				</button>
			</div>
			<div className="overflow-hidden rounded-2xl border border-white/5">
				<table className="w-full text-left border-collapse">
					...
					<tbody>
						{courses.map(function (course) { return (<tr key={course.id} className="border-t border-white/5 hover:bg-white/[0.03] transition-colors group">
								...
								<td className="p-5">
									<span className={"px-3 py-1 rounded-full text-[10px] uppercase font-black tracking-wider ".concat(course.status === "published"
...
var MilestoneQueue = function () {
    ...
    ]), submissions = _a[0], setSubmissions = _a[1];
    var handleAction = function (id, action) {
        if (action === "reject") {
            var reason = prompt("Mandatory reason for rejection:");
            if (!reason)
                return;
        }
        setSubmissions(submissions.filter(function (s) { return s.id !== id; }));
        alert("Milestone #".concat(id, " ").concat(action, "d!"));
    };
    return (<section className="glass-card p-10 rounded-[2.5rem]">
			...
			<div className="grid grid-cols-1 gap-6">
				{submissions.map(function (sub) { return (<div key={sub.id} className="p-6 rounded-2xl bg-white/5 border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-brand-blue/30 transition-all group">
						...
						<div className="flex gap-3">
							<button className="px-6 py-2.5 bg-brand-emerald/10 text-brand-emerald rounded-full font-bold text-xs hover:bg-brand-emerald border border-brand-emerald/30 hover:text-black transition-all uppercase tracking-widest" onClick={function () { return handleAction(sub.id, "approve"); }}>
								Approve
							</button>
							<button className="px-6 py-2.5 bg-brand-purple/10 text-brand-purple rounded-full font-bold text-xs hover:bg-brand-purple border border-brand-purple/30 hover:text-white transition-all uppercase tracking-widest" onClick={function () { return handleAction(sub.id, "reject"); }}>
								Reject
							</button>
						</div>
					</div>); })}
...
var UserLookup = function () {
    ...
    return (<section className="glass-card p-10 rounded-[2.5rem]">
			...
			<div className="flex gap-4 mb-12 max-w-2xl">
				<input type="text" placeholder="Enter wallet address (G...)" className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-brand-cyan/50 focus:bg-white/[0.08] transition-all text-sm font-mono" value={search} onChange={function (e) { return setSearch(e.target.value); }}/>
				<button className="px-8 py-4 bg-linear-to-r from-brand-cyan to-brand-blue rounded-2xl font-black text-sm uppercase tracking-widest hover:scale-[1.02] shadow-xl shadow-brand-cyan/10 transition-transform" onClick={handleSearch}>
					Lookup
				</button>
			</div>
			{userData && (<div className="p-10 rounded-3xl bg-linear-to-br from-white/5 to-transparent border border-white/10 relative overflow-hidden group">
...
var TreasuryControls = function () {
    ...
    return (<section className="glass-card p-10 rounded-[2.5rem]">
			<div className="flex justify-between items-center mb-12">
				...
				<button className={"px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest transition-all shadow-xl ".concat(isPaused
            ? "bg-brand-blue text-white shadow-brand-blue/30"
            : "bg-orange-500/10 text-orange-400 border border-orange-500/30 hover:bg-orange-500 shadow-orange-500/10 hover:text-black")} onClick={function () { return setIsPaused(!isPaused); }}>
...
var ContractInfo = function () {
    ...
    return (<section className="glass-card p-10 rounded-[2.5rem]">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
				{contracts.map(function (c) { return (<div key={c.name} className="p-8 rounded-3xl bg-linear-to-br from-white/5 to-transparent border border-white/5 hover:border-brand-cyan/20 hover:scale-[1.02] transition-all group">
