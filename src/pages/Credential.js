...
var Credential = function () {
    var nftId = (0, react_router_dom_1.useParams)().nftId;
    var _a = (0, react_1.useState)(false), copySuccess = _a[0], setCopySuccess = _a[1];
    ...
    var copyToClipboard = function () {
        void navigator.clipboard.writeText(window.location.href).catch(function () { });
        setCopySuccess(true);
        setTimeout(function () { return setCopySuccess(false); }, 2000);
    };
    return (<div className="py-20 px-6 min-h-screen flex flex-col items-center gap-16 text-white relative overflow-hidden">
			{/* Background Glows */}
			...
			<div className="iridescent-border p-[1px] rounded-[3rem] shadow-2xl animate-in fade-in zoom-in duration-1000">
				<div className="glass-card w-full max-w-5xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-white/10">
					...
					<div className="md:w-5/12 relative aspect-square md:aspect-auto group">
						<img src={nft.artworkUrl} alt="ScholarNFT Badge" className="w-full h-full object-cover opacity-90 group-hover:scale-110 transition-transform duration-1000"/>
					...
					<div className="md:w-7/12 p-16 flex flex-col justify-center">
						...
						<a href={nft.stellarExpertUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#00d2ff] hover:underline">
			...
			<div className="flex flex-wrap justify-center gap-6 animate-in slide-in-from-bottom-8 duration-1000 delay-300">
				<a href={"https://twitter.com/intent/tweet?text=I've just earned my ".concat(nft.programName, " credential on @LearnVault!")} className="px-10 py-4 bg-[#1d9bf0] text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-[#1d9bf0]/20">
					Share to Twitter / X
				</a>
				<button onClick={copyToClipboard} className="px-10 py-4 glass text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-white/10 hover:scale-105 active:scale-95 transition-all border border-white/10">
