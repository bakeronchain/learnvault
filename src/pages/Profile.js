...
var Profile = function () {
    ...
    if (isLoading) {
        ...
        return (<div className="p-12 max-w-6xl mx-auto text-white animate-in fade-in slide-in-from-bottom-8 duration-1000">
				<SkeletonLoader_1.ProfileSkeleton />
			</div>);
    }
    return (<div className="p-12 max-w-6xl mx-auto text-white animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <SkeletonLoader_1.NoCredentialsEmptyState />) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
						{user.nfts.map(function (nft, index) { return (<react_router_dom_1.Link to={"/credentials/".concat(nft.id)} key={nft.id} className="glass-card rounded-[2.5rem] overflow-hidden hover:border-brand-cyan/40 hover:-translate-y-3 transition-all duration-500 group animate-in fade-in zoom-in duration-700" style={{ animationDelay: "".concat(index * 150, "ms") }}>
								<div className="relative aspect-square overflow-hidden mb-2">
									<img src={nft.artwork} alt={nft.program} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 opacity-80 group-hover:opacity-100"/>
